...
/**
 * In-memory wiring for the analytics screen: includes the analytics
 * handlers plus the minimal command bus to seed categories + transactions
 * via the same paths the production app uses.
 */
class StatsTestFixture(
    ...
    suspend fun seedCategory(
        name: String,
        kind: String = "EXPENSE",
        nature: String? = "VARIABLE",
        essentiality: String? = "DISCRETIONARY",
    ): String {
        ...
        commandBus.dispatch(
            CreateCategoryCommand(
                ...
                color = "#1976D2",
                icon = "label",
                nature = nature.takeIf { kind == "EXPENSE" },
                essentiality = essentiality.takeIf { kind == "EXPENSE" },
            )
        )
