...
| **F7 — Widgets y accesos rápidos** | Deps **Glance** + widget "Añadir rápido" (Material You, deep-link); App Shortcuts estáticos+dinámicos (`ShortcutManagerCompat`, conceptos top); deep-link `transactions/new?type=&concept=` con espera tras desbloqueo | ✅ **verificado en emulador**: **widget colocado en el escritorio y probado en vivo** — renderiza "Añadir rápido" + baldosas − Gasto / + Ingreso; tap "− Gasto" abre el editor con **Gasto** preseleccionado tras desbloqueo. Shortcuts estáticos registrados (`dumpsys`). Warm-start del deep-link no cubierto (solo arranque en frío). `TileService` post-MVP. |
